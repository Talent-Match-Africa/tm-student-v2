...
export function OpportunityFilters({ filters, isPending, onNavigate, type }: Props) {
  ...
  return (
    <AdminFilterShell actionCount={2} actions={<><AuthButton className={styles.utilityButton} icon={FilterHorizontalIcon} isLoading={isPending} type="submit">Apply</AuthButton><AuthButton className={styles.utilityButton} disabled={!filters.search && filters.status === "ACTIVE" && !filters.workFlexibility && !filters.industrySector && !filters.location && filters.ordering === "-created_at"} icon={Cancel01Icon} onClick={reset} variant="secondary">Reset</AuthButton></>} ariaLabel="Opportunity filters" onSubmit={submit}>
